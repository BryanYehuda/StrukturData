...
{
    struct LLNode *temp = (struct LLNode *)
...
void verticalUtility(struct Node *root, LLNode *sumNode)
{
    ...
    if (root->left)
    {
        ...
        verticalUtility(root->left, sumNode->prev);
    }
    ...
    if (root->right)
    {
        ...
        verticalUtility(root->right, sumNode->next);
    }
}
